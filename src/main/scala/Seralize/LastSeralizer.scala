...
object LastSeralizer {
  ...
  def seralize(rows: Iterator[String]): Iterator[JsonExplorerType] = {
    ...
    // Initalize types for comparison
    ...
    // gson needed only once per partition
    ...
    val t:Iterator[Int] = rows.map(row => {
      val s = gson.fromJson(row, MapType)
      s.size()
    })
    t.next()
    ???
